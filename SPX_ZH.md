...
✅  开放格式`190`，基本数据，webp编码（更关注压缩率时适用）
...
| 8~n | bytes  | `*`数据 | length,webp([x0,y0,z0,255...x1,y1,z1,255...x2,y2,z2,255...]), length,webp([sx,sy,sz,255...]), length,webp([r,g,b,a...]), length,webp([rx,ry,rz,255...], length,webp([il,iu,0,255...]) |

- `x,y,z` 坐标，24位编码
- `sx,sy,sz` 缩放，单字节编码
- `r,g,b,a` 颜色和透明度，单字节编码
- `rx,ry,rz` 旋转，单字节编码
- `il,iu` 调色板索引的低位和高位，无调色板时不存在

---
 


✅  开放格式`8`，球谐系数调色板（基本数据不使用webp编码时自动使用）
...
| 8~n | bytes | `*`数据 | [sh0,sh1,sh2...sh15,sh0,sh1,sh2...sh15,...] |

- `sh0,sh1,sh2...sh15` 球谐系数(r,g,b,255)，像素单位

---



✅  开放格式`9`，球谐系数调色板，webp编码（基本数据使用webp编码时自动使用）
...
| 8~n      | bytes  | `*`数据       | webp([sh0,sh1,sh2...sh15,sh0,sh1,sh2...sh15,...]) |
