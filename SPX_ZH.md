...
|       |   bit 0 | 标识1，是否大场景  | 默认小场景`0` |
|       |   bit 1 | 标识2，是否倒立    | 默认非倒立`0` |
|       |   bit 2 | 标识3，预留        | 预留 |
|       |   bit 3 | 标识4，预留        | 预留 |
|       |   bit 4 | 标识5，预留        | 预留 |
|       |   bit 5 | 标识6，预留        | 预留 |
...
|     | bit1~bit3  | `*`压缩类型       | 共3位用于表示数据内容的压缩类型，0:gzip(默认)，1:zstd |
