...
| 54~55 | uint16  | 数据分类最大值     | 当数据有分类时需写入最大值，默认`0` |
| 56–63 |         | 预留              |         |
...
|     | bit1~bit3  | `*`压缩类型       | 共3位用于表示数据内容的压缩类型，0:gzip(默认) |
...
|----------|------|------|------|
| 0~3   | uint32 | `*`数量 | 高斯点数 |
| 4~7   | uint32 | `*`格式识别号 | `16`基本数据 |
| 8~11  | uint32 | `*`minX | 数据块内的 x 坐标最小值 |
| 12~15 | uint32 | `*`maxX | 数据块内的 x 坐标最大值 |
| 16~19 | uint32 | `*`minY | 数据块内的 y 坐标最小值 |
| 20~23 | uint32 | `*`maxY | 数据块内的 y 坐标最大值 |
| 24~27 | uint32 | `*`minZ | 数据块内的 z 坐标最小值 |
| 28~31 | uint32 | `*`maxZ | 数据块内的 z 坐标最大值 |
| 32~n | bytes | `*`数据 | x0...y0...z0...x1...y1...z1...sx...sy...sz...r...g...b...a...rx...ry...rz... |
