...
| 8~n | bytes  | `*`数据 | length,webp([x0,y0,z0,255...x1,y1,z1,255...x2,y2,z2,255...]), length,webp([sx,sy,sz,255...]), length,webp([r,g,b,a...]), length,webp([rx,ry,rz,ri...], length,webp([p0,p1,0,255...]) |

- `x,y,z` 坐标，24位编码
- `sx,sy,sz` 缩放，单字节编码
- `r,g,b,a` 颜色和透明度，单字节编码
- `rx,ry,rz,ri` 旋转，ri为最大值的位置+252，单字节编码
- `p0,p1` 调色板索引的低位和高位，无调色板时不存在
